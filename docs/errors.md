...
| `SPLIT_PARTS_INVALID` | The parts a charge was to be split into do not account for its bill: a line named twice, a line named that is not on it, or a line left out. Every line has to land in exactly one part, because a part's amount is cut from the charge in proportion to the lines it holds -- so a line belonging to nobody is money the parts would not sum to. | `unknownItemIds`, `duplicatedItemIds`, or `unplacedItemIds` and `unplacedItemNames`, depending on which it was. |
