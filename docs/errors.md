...
| `SPLIT_RULE_NOT_EDITABLE` | A rename, a restatement, a written history or a deletion was asked for on a rule the group was given rather than wrote -- the one it holds per member, putting the whole amount on them. It exists for every member without anybody creating it, and an expense may name it the same way, so what it says is fixed: changing it would move money on expenses recorded under a division nobody chose. A group wanting its own is one `POST /split-rules` away, and that one is editable. | `splitRuleId`, `allForUserId`. |
