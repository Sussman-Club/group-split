...
`groups settle`. `groups settle-up` reads the balances for the same reason, so the
confirmation lists the repayments it is about to write rather than describing them. `bank unlink`, `categories delete` and `split-rules delete` do the same.
